...
will be used along with the yield curve factors, $X_t$. These have been calculated in Section \@ref(spot-rates) and Section \@ref(yield-curve-factors), and the results from there are used here.
...
future rates, and is simply used to gather intuition about past rates. Nevertheless,
it is interesting to see how well the yield curve factors explain the variation.

```{r spot-rate-reg-chart, fig.width=10, echo=FALSE, fig.pos = 'H', fig.cap="Regression in-sample realized VS predicted.", fig.subcap="All maturities essentially fit perfectly.", warning=FALSE, fig.align='center', cache=TRUE}
...
decompose and plot the spot curve at a few particular points in time. The procedure for this involved
...
decomposed spot rate for January 2012 and January 1981 side by side is shown
...
5026 regressions, which on this computer can be run in parallel in ~8 seconds.
...
over time, with the exception of curvature. The curvature of the 1 year has begun to rise up from `-0.5` to around `-0.25` in recent years. This change over time is not refected in the `-0.428` curvature estimate we get from running the model
over the full time period, and might offer other interesting insights. The 10 year curvature coefficient follows a similar pattern,
...
```{r coef-stability, fig.width=10, echo=FALSE, fig.pos = 'H', fig.cap="Coefficient stability for 1, 5, and 10 year spot rates. 100 day rolling window.", warning=FALSE, fig.align='center', fig.height=8, cache=TRUE}
